...
void periodic(ctermui_screen_t *screen_p) {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    ctermui_component_t progress_bar =
        ctermui_layout_find_component(ctermui_layout_find(root, id), id);
...
int main() {
  ...
  for (size_t i = 0; i < 10; i++) {
    if (i % 2 == 0) {
      ctermui_layout_add_child(padding_h,
                               ctermui_layout_new("padding", LEAF, 10));
      continue;
    }
    char id[100];
    sprintf(id, "progress_bar_%zu", i);
    ctermui_component_t progress_bar = ctermui_new_progress_bar(
        id, CTERMUI_GREEN, CTERMUI_CYAN, 100, i, "Loading...", CTERMUI_WHITE,
        CTERMUI_HORIZONTAL);
    ctermui_layout_t progress_layout = ctermui_layout_new(id, LEAF, 10);
    ctermui_layout_add_component(progress_layout, progress_bar);
