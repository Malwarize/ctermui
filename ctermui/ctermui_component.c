...
ctermui_component_t ctermui_new_button(
        char *id,
        char *text,
        size_t align,
        int8_t text_color,
...
void ctermui_scatter_plot_draw(ctermui_screen_t s, ctermui_component_t c) {
    ...
    ctermui_pencil_draw_line(
            s->buffer, CTERMUI_VERTICAL, x_plot_starting, y_plot_starting, y_plot_height, CTERMUI_VERTICAL_LINE,
            plt->fg_color, plt->bg_color
    );
    ctermui_pencil_draw_line(
            s->buffer, CTERMUI_HORIZONTAL, x_plot_starting, y_plot_starting + y_plot_height - 1, x_plot_width,
            CTERMUI_HORIZONTAL_LINE, plt->fg_color, plt->bg_color
    );
    ...
    for (size_t i = 0; i < plt->size; ++i) {
        ...
        ctermui_pencil_draw_char(
                s->buffer, x_position, y_plot_starting + y_plot_height - 1, CTERMUI_VERTICAL_LINE, plt->fg_color,
                plt->bg_color, 0
        );
        ctermui_pencil_draw_char(
                s->buffer, x_plot_starting, y_position, CTERMUI_HORIZONTAL_LINE, plt->fg_color, plt->bg_color, 0
        );
        ...
        ctermui_pencil_draw_text(
                s->buffer, label_pos, c->absolute_y + c->absolute_height - 1, x_label, plt->fg_color, plt->bg_color
        );
    }
    ...
    for (size_t i = 0; i < plt->size; ++i) {
        ...
        ctermui_pencil_draw_char(
                s->buffer, x_position, y_position, plt->point_symbol, plt->point_color, plt->bg_color, 0
        );
        if (plt->line_linking) {
            if (i > 0) {
                ...
                if (min_x == max_x) {
                    ...
                    scaled_x_prev = (plt->xvalues[i - 1] - min_x) / (max_x - min_x) * (x_plot_width - 1);
                    x_position_prev = x_plot_starting + (int) scaled_x_prev;
                }
                ...
                if (min_y == max_y) {
                    ...
                    scaled_y_prev = (plt->yvalues[i - 1] - min_y) / (max_y - min_y) * (y_plot_height - 1);
                    y_position_prev = y_plot_starting + y_plot_height - 1 - (int) scaled_y_prev;
                }
                ctermui_pencil_draw_line_with_points(
                        s->buffer, x_position_prev, y_position_prev, x_position, y_position, plt->point_color,
                        plt->bg_color, plt->point_symbol
                );
            }
        }
    }
