...
void ctermui_component_draw_button(ctermui_screen_t s, ctermui_component_t c) {
  ...
  ctermui_pencil_solid_background(s->buffer, c->absolute_x, c->absolute_y,
                                  frame_width, frame_height, button->bg_color);
  ctermui_pencil_draw_text(s->buffer,
                           c->absolute_x + (frame_width - text_width) / 2,
                           c->absolute_y + (frame_height - 1) / 2, button->text,
...
void ctermui_component_draw_label(ctermui_screen_t s, ctermui_component_t c) {
  ...
  ctermui_pencil_draw_text(s->buffer, c->absolute_x, c->absolute_y, text->text,
...
void ctermui_component_draw_frame(ctermui_screen_t s, ctermui_component_t c) {
  ...
  ctermui_pencil_draw_rect(s->buffer, c->absolute_x, c->absolute_y,
                           c->absolute_width, c->absolute_height, frame->color,
                           frame->bg_color);
}
...
void ctermui_component_draw_solid_background(ctermui_screen_t s,
  ...
  for (size_t y = c->absolute_y; y < c->absolute_y + c->absolute_height; ++y) {
    for (size_t x = c->absolute_x; x < c->absolute_x + c->absolute_width; ++x) {
...
void ctermui_component_draw_soft_background(ctermui_screen_t s,
  ...
  ctermui_pencil_bucket(s->buffer, c->absolute_x, c->absolute_y,
                        c->absolute_width, c->absolute_height,
                        background->color);
}
...
void ctermui_calculate_absolute_position(ctermui_component_t c, size_t parent_x,
...
void ctermui_component_draw_progress_bar(ctermui_screen_t s,
  ...
  if (progress_bar->orientation == CTERMUI_HORIZONTAL) {
    ...
    ctermui_pencil_solid_background(s->buffer, c->absolute_x, c->absolute_y,
                                    progress_width, c->absolute_height,
                                    progress_bar->bar_color);
    size_t text_width = strlen(progress_bar->text);
    size_t text_x = c->absolute_x + (c->absolute_width - text_width) / 2;
    size_t text_y = c->absolute_y + (c->absolute_height - 1) / 2;
    ...
    ctermui_pencil_solid_background(
        s->buffer, c->absolute_x,
        c->absolute_y + c->absolute_height - progress_height, c->absolute_width,
...
void ctermui_barchart_update_values(ctermui_component_t c, int *values,
  ...
  if (c->type != BARCHART) {
    fprintf(stderr, "ctermui_pencil_draw_barchart: invalid component type\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (barchart->orientation == CTERMUI_HORIZONTAL) {
    ...
    for (size_t i = 0; i < barchart->values_count; ++i) {
      ...
      label->calculate_absolute_position(
          label, x_position, parent_y + parent_height - label->absolute_height,
          bar_width, label->absolute_height);
...
void ctermui_scatter_plot_draw(ctermui_screen_t s, ctermui_component_t c) {
  ...
  for (size_t i = 0; i < plt->size; ++i) {
    ...
    ctermui_pencil_draw_text(s->buffer, c->absolute_x, y_position, y_label,
    ...
    ctermui_pencil_draw_text(s->buffer, label_pos,
                             c->absolute_y + c->absolute_height - 1, x_label,
                             plt->fg_color, plt->bg_color);
  }
...
void ctermui_text_input_draw(ctermui_screen_t s, ctermui_component_t c) {
  if (c->type != TEXT_INPUT) {
    fprintf(stderr, "ctermui_pencil_draw_text_input: invalid component type\n");
    exit(EXIT_FAILURE);
  }
  ...
  ctermui_pencil_draw_text(
      s->buffer, c->absolute_x, c->absolute_y + (frame_height - 1) / 2,
      text_input->text, text_input->text_color, text_input->bg_color);
}
...
ctermui_component_t ctermui_new_custom_component(
    ...
    void (*calculate_absolute_position)(
        ...
        size_t parent_x, size_t parent_y, size_t parent_width,
        size_t parent_height),
    void *core_component) {
  ctermui_component_t c =
      (ctermui_component_t)malloc(sizeof(struct ctermui_component));
  if (!c) {
